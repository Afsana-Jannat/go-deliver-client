...
//         console.log("sender center", parcel.sender_district)

//         try {
//             const res = await axiosSecure.get("/riders/available", {
//                 params: {
//                     district: parcel.sender_district, // match with rider.district
//                 },
//             });
//             setRiders(res.data);
//         } catch (error) {
...
//                                             className="btn btn-sm bg-[#4f40ae] text-white">
...
const AssignRider = () => {
    ...
    // ✅ Updated mutation with parcelDetails refresh
    const { mutateAsync: assignRider } = useMutation({
        mutationFn: async ({ parcelId, rider }) => {
            return axiosSecure.patch(`/parcels/${parcelId}/assign`, {
                riderId: rider._id,
                riderEmail: rider.email,
                riderName: rider.name,
            });
        ...
        onSuccess: () => {
            // 🔥 Refresh list of assignable parcels
            queryClient.invalidateQueries(["assignableParcels"]);

            // 🔥 Refresh parcel tracking details (important!)
            queryClient.invalidateQueries(["parcelDetails"]);
    ...
    const openAssignModal = async (parcel) => {
        ...
        try {
            const res = await axiosSecure.get("/riders/available", {
                params: {
                    district: parcel.sender_district,
                },
            });
            setRiders(res.data);
        } catch (error) {
